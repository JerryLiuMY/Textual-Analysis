...
def build_word(sub_file_rich):
    ...
    print(f"{datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')} Working on {sub_file_rich}")
    def func(df): return df["text"] if df["title"] is np.nan else " ".join([df["title"], df["text"]])
    sub_word_matrix = sub_df_rich.apply(func, axis=1).apply(lambda _: [_.count(word) for word in full_dict])
